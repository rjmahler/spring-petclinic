...
class PetController {
	...
	public String processUpdateForm(@Valid Pet pet, BindingResult result, Owner owner, ModelMap model,
		...
		if (result.hasErrors() || petNameExists(owner, pet, petId, result) || isInvalidBirthDate(pet, result)) {
			pet.setOwner(owner);
			model.put("pet", pet);
			return VIEWS_PETS_CREATE_OR_UPDATE_FORM;
		}
	...
	private boolean isInvalidBirthDate(Pet pet, BindingResult result) {
		boolean validDate = pet.getBirthDate().isBefore(LocalDate.now());
		if (!validDate) {
			result.rejectValue("birthDate", "invalid_birth_date", "invalid date");
		}
		return !validDate;
	}
